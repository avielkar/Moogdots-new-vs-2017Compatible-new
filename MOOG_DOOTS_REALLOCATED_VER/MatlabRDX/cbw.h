 ...
 * Name:   CBW.H
 *
 * C/C++ Windows header for Computer Boards Universal Library
 *
 * (c) Copyright 1996-1998, Computer Boards
 * All rights reserved.
 *
 * This header file should be included in all C/C++ programs that will
 * call the Computer Boards Universal Library from Windows.
 *
 *
 **************************************************************************/
...
#define AMBIGPCMSLOTREF         122   /* More than one CBI PCM card in slot */
...
#define IRQNOTAVALIABLE         134	  /* IRQ in being used by another device */	
 ...
 */
    ...
    int EXTCCONV cbDOutScan (int BoardNum, int PortNum, long Count, long *Rate,
                             HGLOBAL MemHandle, int Options);
    void EXTCCONV cbErrHandling (int ErrReporting, int ErrHandling);
    ...
    int EXTCCONV cbGetStatus (int BoardNum, short *Status, long *CurCount,
                              long *CurIndex);
