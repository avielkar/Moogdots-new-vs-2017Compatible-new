...
private:
    ...
    enum
    {
        eMAX_NUM_COMMANDS = 40000,                // max number of commands in a script
        eMAX_NUM_COMMAND_ARGUMENTS = 100,        // max number of arguments for a single command
        eMAX_NUM_COMMAND_ARGUMENT_LENGTH = 100   // max argument length
    };
...
public:
	CScriptFile(CMBCInterface* pMBC);
    ~CScriptFile();
