...
cntrnext = trial(activeStair,activeRule).list(trial(activeStair,activeRule).cntr+1);
...
if data.configinfo(i).status == 2
    ...
    azPn(1,1) = crossvals(cntrVaryingNext,i1);
    azPn(2,1) = crossvalsGL(cntrVaryingNext,i1);
...
orin(1)= -0.5*dist(1,1)*cos(azPn(1,1)*pi/180);
orin(2)= -0.5*dist(1,1)*sin(azPn(1,1)*pi/180);
orin(3)= data.configinfo(i).parameters(3);
...
%M(16).name = 'FP_ON';
%M(16).data = 1;
M(16).name = 'M_ORIGIN_NEXT';
M(16).data = [orin(1) orin(2) orin(3)];
