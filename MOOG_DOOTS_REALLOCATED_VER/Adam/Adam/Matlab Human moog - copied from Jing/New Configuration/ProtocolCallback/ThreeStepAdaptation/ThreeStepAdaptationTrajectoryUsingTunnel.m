%Three Step Adapte heading discrimination protocol Using Tunnle. Hacker: Jian from Jing for Mandy 2/02/2010
function [M] = ThreeStepAdaptationTrajectoryUsingTunnel(appHandle)
...
iTunnel = strmatch('ENABLE_TUNNEL',{char(data.configinfo.name)},'exact');
if stim_type == 1  %vestibula only
    data.configinfo(iTunnel).parameters = 0;
else   %Combine & Visual only
    data.configinfo(iTunnel).parameters = 1;
end
