...
% Stereo and monocular trials interleaved
i = strmatch('TRAINING',{char(data.configinfo.name)},'exact');
if data.configinfo(i).parameters == 1
    i = strmatch('STAR_LEYE_COLOR',{char(data.configinfo.name)},'exact');
    if rem(activeStair,2)>0 % if activeStair is odd
        data.configinfo(i).parameters = [0 0 0];
    else % if even
        data.configinfo(i).parameters = [1 0 0];
    end
end
...
if data.configinfo(i).status == 2
    i1 = strmatch('Wall Distance (Z)',{char(varying.name)},'exact');
    WallDistance = crossvals(cntr,i1);
elseif data.configinfo(i).status == 3
    WallDistance = across.parameters(activeStair);
elseif data.configinfo(i).status == 4
    WallDistance = within.parameters(cntr);
else
    WallDistance = data.configinfo(i).parameters;
end
% Hack to get diff distance for diff vest vels
WallDistance=WallDistance(activeStair) 
...
% eye2zeropt=69; % Constant calculated offline

% Set distance by changing wall origin
i = strmatch('WALL_ORIGIN',{char(data.configinfo.name)},'exact');
data.configinfo(i).parameters(2) =  WallDistance - eye2zeropt;

% See if we need this to foce wall_origin to update
% outString = ['WALL_ORIGIN ',data.configinfo(i).parameters];
% if connected
...
density_constant=160;
i = strmatch('WALL_DIMS_N_DENSITY',{char(data.configinfo.name)},'exact');
% i = strmatch('WALL_DIMS',{char(data.configinfo.name)},'exact');
WallDims=WallDistance*4.5; % Constant calculated offline
% data.configinfo(i).parameters = [WallDims WallDims];
% i = strmatch('WALL_DENSITY',{char(data.configinfo.name)},'exact');
WallDensity=density_constant/(WallDistance*WallDistance); % Constant calculated offline
WallDims*WallDims*WallDensity % Check: this should be a constant
% Randomize
% RndDensity=rand*2
% if RndDensity>1
%     WallDensity=WallDensity*RndDensity;
% else
%     WallDensity=WallDensity*((RndDensity+1)/2);
% end
data.configinfo(i).parameters = [WallDims WallDims WallDensity];

% Set dims and density depending on distance for 2nd interval
WallDistance2I = DepthOffset
% WallDistance2I = WallDistance + (WallDistance*DepthOffset)
i = strmatch('WALL_DIMS2I',{char(data.configinfo.name)},'exact');
WallDims2I=WallDistance2I*4.5; % Constant calculated offline
data.configinfo(i).parameters = [WallDims2I WallDims2I];
i = strmatch('WALL_DENSITY2I',{char(data.configinfo.name)},'exact');
WallDensity2I=density_constant/(WallDistance2I*WallDistance2I); % Constant calculated offline
WallDims2I*WallDims2I*WallDensity2I % Check: this should be a constant
% Randomize
% RndDensity=rand*2
% if RndDensity>1
%     WallDensity2I=WallDensity2I*RndDensity;
% else
%     WallDensity2I=WallDensity2I*((RndDensity+1)/2);
% end
data.configinfo(i).parameters = WallDensity2I;

i = strmatch('WALL_ORIGIN2I',{char(data.configinfo.name)},'exact');
data.configinfo(i).parameters = WallDistance2I - eye2zeropt;
...
% 2nd Interval % this code doesn't matter b/c movement is nulled below
if motiontype == 3
...
%     % 2nd interval
%     dM2 = GenTrapezoid(dur(1,ord(2)), dist(1,ord(2)), .2, .8, f);
%     %dM1 = cumtrapz(vM1);
%     dM2 = abs(dist(1,ord(2)))*dM2(1:end-1)/max(abs(dM2));
%     dGL2 = GenTrapezoid(dur(2,ord(2)), dist(2,ord(2)), .2, .8,f);
%     %dGL1 = cumtrapz(vGL1);
%     dGL2 = abs(dist(2,ord(2)))*dGL2(1:end-1)/max(abs(dGL2));
end
...
if motiontype == 1    
    ...
    % POGEN changed this: 2nd interval uses oppamp instead of amp to reverse
    % direction
    xM2 = -sin(oppamp(1,ord(2)))*sin(az(1))*cos(tilt(1)) + cos(oppamp(1,ord(2)))*...
         (cos(az(1))*cos(el(1))+sin(az(1))*sin(tilt(1))*sin(el(1)));
    yM2 = sin(oppamp(1,ord(2)))*cos(az(1))*cos(tilt(1)) + cos(oppamp(1,ord(2)))*...
         (sin(az(1))*cos(el(1))-cos(az(1))*sin(tilt(1))*sin(el(1)));
    zM2 = -sin(oppamp(1,ord(2)))*sin(tilt(1)) - cos(oppamp(1,ord(2)))*sin(el(1))*cos(tilt(1));


    xGL2 = -sin(oppamp(2,ord(2)))*sin(az(2))*cos(tilt(2)) + cos(oppamp(2,ord(2)))*...
          (cos(az(2))*cos(el(2))+sin(az(2))*sin(tilt(2))*sin(el(2)));
    yGL2 = sin(oppamp(2,ord(2)))*cos(az(2))*cos(tilt(2)) + cos(oppamp(2,ord(2)))*...
          (sin(az(2))*cos(el(2))-cos(az(2))*sin(tilt(2))*sin(el(2)));   
    zGL2 = -sin(oppamp(2,ord(2)))*sin(tilt(2)) - cos(oppamp(2,ord(2)))*sin(el(2))*cos(tilt(2)); 
...
%     surgeGL2 = dGL2*xGL2*0 + surgeGL1(end); % Null second interval movement
    surgeGL = [surgeGL1 surgeRestGL surgeGL2]; % Move visual trajectory towards/away from wall
