...
%==== Special parameters for FPlane   Jian 05/06/2011=========
i = strmatch('FPLANE_YAW',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 2
    i1 = strmatch('FPlane Yaw',{char(varying.name)},'exact');
...
i = strmatch('FPLANE_ROLL',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 2
    i1 = strmatch('FPlane Roll',{char(varying.name)},'exact');
...
if motiontype == 3 % 2I vars required as well
    ...
    %==== Special parameters for FPlane   Jian 05/06/2011=========
    i = strmatch('FPLANE_YAW_2I',{char(data.configinfo.name)},'exact');
    if data.configinfo(i).status == 2
        i1 = strmatch('FPlane Yaw 2nd Int',{char(varying.name)},'exact');
    ...
    i = strmatch('FPLANE_ROLL_2I',{char(data.configinfo.name)},'exact');
    if data.configinfo(i).status == 2
        i1 = strmatch('FPlane Roll 2nd Int',{char(varying.name)},'exact');
...
M(13).name = 'FPLANE_DUR';
M(13).data = dur(1,ord(1));
M(14).name = 'FPLANE_DUR_2I';
M(14).data = dur(1,ord(2));
M(15).name = 'FPLANE_DELAY_2I';
M(15).data = delay(1);
M(16).name = 'FPLANE_YAW';
M(16).data = fplane_yaw(1,ord(1));
M(17).name = 'FPLANE_ROLL';
M(17).data = fplane_roll(1,ord(1));
M(18).name = 'FPLANE_YAW_2I';
M(18).data = fplane_yaw(1,ord(2));
M(19).name = 'FPLANE_ROLL_2I';
M(19).data = fplane_roll(1,ord(2));
M(20).name = 'ENABLE_FPLANE_2I';
if motiontype == 3
    M(20).data = 1;
else
    M(20).data = 0;
