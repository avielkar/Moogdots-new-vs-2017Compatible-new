...
if data.configinfo(i).status == 2
    i1 = strmatch('Disparity',{char(varying.name)},'exact');
    disparity = crossvals(cntr,i1);
else
    disparity = data.configinfo(i).parameters.moog;
end


if (disparity == 0) %%% zero disparity; both headings should be same
    clear a_config 
    a_config = randperm(length(heads));
    vest = heads(a_config(end));
    vis = vest;
    amps(1,1) = vest;
    amps(2,1) = vis;
...
if (disparity ~= 0)
    while (kkk <= 100)
        clear a_config vest vis
        a_config = randperm(length(heads));
        vest = heads(a_config(end));
        clear a_config
        a_config = randperm(length(heads));
        vis = heads(a_config(end));
        if (ceil(abs(vest-vis))==disparity )
            kkk = 10000;
        end
    end
    amps(1,1) = vest;
...
display('amps(1,1) ='); amps(1,1), display('amps(2,1) ='); amps(2,1), display('disparity ='); disparity,
if motiontype == 1
