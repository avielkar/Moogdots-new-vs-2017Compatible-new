...
cause = 1;
heads = [-25 -22.5 -20 -17.5 -15 -12.5 -10 -7.5 -5 -2.5 2.5 5 7.5 10 12.5 15 17.5 20 22.5 25];
clear amps(1,1) amps(2,1) amps(1,2) amps(2,2);
clear a_config 

a_config = randperm(length(heads));
amps(1,1)= heads(a_config(2));
amps(2,1) = amps(1,1);

i = strmatch('VESTVIS_TYPE',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 2
    i1 = strmatch('VestVis Type',{char(varying.name)},'exact');
    vestvis_stimtype = crossvals(cntr,i1);
else
    vestvis_stimtype = data.configinfo(i).parameters.moog;
end


if (vestvis_stimtype == 1) 
...
i = strmatch('VEST_LENGTH',{char(data.configinfo.name)},'exact');
data.configinfo(i).parameters = dist(1,1); 
setappdata(appHandle,'protinfo',data);

i = strmatch('VISUAL_LENGTH', {char(data.configinfo.name)},'exact');
data.configinfo(i).parameters = dist(2,1);
setappdata(appHandle,'protinfo',data);
...
display('amps(1,1) ='); amps(1,1), display('amps(2,1) ='); amps(2,1), display('cause ='); cause,
if motiontype == 1
    sprintf('amps(1,1)=%f amps(2,1)=%f',amps(1,1), amps(2,1))
% %     dir_1I = amps(1,1)
% % else
% %     if HR
