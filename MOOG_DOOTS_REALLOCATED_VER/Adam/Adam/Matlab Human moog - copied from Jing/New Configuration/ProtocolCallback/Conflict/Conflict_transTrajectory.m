...
% i = strmatch('STIMULUS_CONDITION',{char(data.configinfo.name)},'exact');
% if data.configinfo(i).status == 2
%     i1 = strmatch('Stimulus Condition',{char(data.condvect.name)},'exact');
%     stim_cond(1,1) = crossvals(cntr,i1);
% else
%     stim_cond(1,1) = data.configinfo(i).parameters.moog;
% 
% end
% 
% i = strmatch('STAR_MOTION_COHERENCE',{char(data.configinfo.name)},'exact');
% if data.configinfo(i).status == 2
%     i1 = strmatch('Star Motion Coherence',{char(data.condvect.name)},'exact');
%     star_motion_coh(1,1) = crossvals(cntr,i1);
% 
% else
%     star_motion_coh(1,1) = data.configinfo(i).parameters.moog;
% end
