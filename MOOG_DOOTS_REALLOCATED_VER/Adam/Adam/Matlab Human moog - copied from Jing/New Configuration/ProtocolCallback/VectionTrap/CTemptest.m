...
% %     elseif data.configinfo(i).status == 4
% %         amps(1,2) = within.parameters.moog(cntr); % Pogen modified to make relative to 1st
% %         amps(2,2) = within.parameters.openGL(cntr);  % Pogen modified to make relative to 1st
% %     else
...
% set origin depending on heading direction and ref plane tilt
if tiltP(1,1)==0
    latpos=sin(amps(1,1)/180*pi)*dist(1,1);
    forpos=cos(amps(1,1)/180*pi)*dist(1,1);
    origin=[-latpos -forpos 0]
else % tilt plane is 90
    verpos=sin(amps(1,1)/180*pi)*dist(1,1);
    forpos=cos(amps(1,1)/180*pi)*dist(1,1);
    origin=[0 -forpos verpos]
end    
ori(1,:) = origin;
ori(2,:) = origin;
i = strmatch('ORIGIN',{char(data.configinfo.name)},'exact');
data.configinfo(i).parameters=origin;
...
i = strmatch('CONFLICT_SIZE',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 2
    i1 = strmatch('Conflict_size',{char(varying.name)},'exact');
    conflict_size = crossvals(cntr,i1);
else
    conflict_size = data.configinfo(i).parameters;
end

%%%conflict_size = data.configinfo(i).parameters;
% % i = strmatch('CONFLICT_ON',{char(data.configinfo.name)},'exact');
% % conflict_on = data.configinfo(i).parameters;
% % conflict_valence=round((rand*3)-1.5); % pick -1, 0, or 1
% % i = strmatch('CONFLICT_VALENCE',{char(data.configinfo.name)},'exact');
% % data.configinfo(i).parameters=conflict_valence;
% % conflict_valence
% setappdata(appHandle,'Conflict_valence',conflict_valence); %This line
% does nothing!
%%%conflict_valence = 1;
%%%conflict=conflict_valence*conflict_size;
% % if conflict_on
% % amps(1,1)=amps(1,1)+conflict; % add conlfict amount to moog heading
% % amps(2,1)=amps(2,1)-conflict; % subtract conlfict amount from vis heading
% % end
conflict = conflict_size
%%%%amps(1,1)=amps(1,1)+conflict; % add conlfict amount to moog heading
amps(2,1)=amps(2,1)-conflict; % subtract conlfict amount from vis heading
setappdata(appHandle,'protinfo',data); % MUST be called to set/save params
...
if motiontype == 1    
    ...
    heaveGL = dGL1*zGL;
% % else
% %     xM1 = xM; yM1 = yM; zM1 = zM; xGL1 = xGL; yGL1 = yGL; zGL1 = zGL;
% %    
% %     xM2 = -sin(amp(1,ord(2)))*sin(az(1))*cos(tilt(1)) +
% %     cos(amp(1,ord(2)))*...
% %          (cos(az(1))*cos(el(1))+sin(az(1))*sin(tilt(1))*sin(el(1)));
% %     yM2 = sin(amp(1,ord(2)))*cos(az(1))*cos(tilt(1)) +
% %     cos(amp(1,ord(2)))*...
% %          (sin(az(1))*cos(el(1))-cos(az(1))*sin(tilt(1))*sin(el(1)));
% %     zM2 = -sin(amp(1,ord(2)))*sin(tilt(1)) -
% %     cos(amp(1,ord(2)))*sin(el(1))*cos(tilt(1));
% % 
% % 
% %     xGL2 = -sin(amp(2,ord(2)))*sin(az(2))*cos(tilt(2)) +
% %     cos(amp(2,ord(2)))*...
% %           (cos(az(2))*cos(el(2))+sin(az(2))*sin(tilt(2))*sin(el(2)));
% %     yGL2 = sin(amp(2,ord(2)))*cos(az(2))*cos(tilt(2)) +
% %     cos(amp(2,ord(2)))*...
% %           (sin(az(2))*cos(el(2))-cos(az(2))*sin(tilt(2))*sin(el(2)));   
% %     zGL2 = -sin(amp(2,ord(2)))*sin(tilt(2)) -
% %     cos(amp(2,ord(2)))*sin(el(2))*cos(tilt(2)); 
% %     
% %     restM = ones(1,f*delay(1)); restGL = ones(1,f*delay(2));
