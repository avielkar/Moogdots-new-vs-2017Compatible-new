function [M] = triTrajectory_RTT(appHandle)

global debug

if debug
    disp('Entering triTrajectory_RTT');
end
...
i = strmatch('ACCEL',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 2
    i1 = strmatch('Acceleration',{char(varying.name)},'exact');
    accel(1,1) = crossvals(cntrVarying,i1);
    accel(2,1) = crossvalsGL(cntrVarying,i1);
elseif data.configinfo(i).status == 3   
    accel(1,1) = across.parameters.moog(activeStair);
    accel(2,1) = across.parameters.openGL(activeStair);
elseif data.configinfo(i).status == 4   
    accel(1,1) = within.parameters.moog(cntr);
    accel(2,1) = within.parameters.openGL(cntr);
else
    accel(1,1) = data.configinfo(i).parameters.moog;
    accel(2,1) = data.configinfo(i).parameters.openGL;
end
...
% calculating distance for specific accel
dist = 0.5 * (accel * 9.81 * 100) .* (dur/2).*dur/2; %in meters
dist = dist * 2; %i.e. the two section == acceleration and deceleration
...
if debug
    disp('Exiting triTrajectory_RTT');
end
