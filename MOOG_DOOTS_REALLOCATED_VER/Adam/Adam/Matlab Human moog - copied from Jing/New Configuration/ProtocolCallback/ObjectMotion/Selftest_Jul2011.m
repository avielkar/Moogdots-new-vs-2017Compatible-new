...
if data.configinfo(i).status == 2
    i1 = strmatch('Heading Change',{char(varying.name)},'exact');
    devs = crossvals(cntr, i1);
elseif data.configinfo(i).status == 3   
    devs = across.parameters(activeStair);
elseif data.configinfo(i).status == 4   
    devs = within.parameters(cntr);
else
    devs = data.configinfo(i).parameters;
end

%%%% For forward or backward motion in one interval.
...
%%%%% Reading in the value of incliniation of the object. 
...
%%%% Reading in the required object elevation.
i = strmatch('OBJECT_ELE',{char(data.configinfo.name)},'exact');
def_obj_ele = data.configinfo(i).parameters;


%%% Creating the required object deviation with respect to the required elevation.
new_obj_ele = def_obj_ele + obj_dev;

%%% Setting the above computed value as the new required object devation
%%% trajectory.
i = strmatch('OBJECT_ELE',{char(data.configinfo.name)},'exact');
data.configinfo(i).parameters = new_obj_ele;
setappdata(appHandle, 'protinfo', data);

%%%% Reading in object distance and creating object trajectory.
...
% % if (activeStair == 8) %%%% Visual + Object
% %     dist(1,1) = 0.000001;
% %     dist(1,2) = 0.000001;
% %     i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
% %     data.configinfo(i).parameters = 1;
...
% % end
% % 
% % if (activeStair == 9)  %%%% Vest + Visual + Object
% %     i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
% %     data.configinfo(i).parameters = 1;
% %     setappdata(appHandle,'protinfo',data);
% %     i = strmatch('SPHERE_FIELD_PARAMS',{char(data.configinfo.name)},'exact');
