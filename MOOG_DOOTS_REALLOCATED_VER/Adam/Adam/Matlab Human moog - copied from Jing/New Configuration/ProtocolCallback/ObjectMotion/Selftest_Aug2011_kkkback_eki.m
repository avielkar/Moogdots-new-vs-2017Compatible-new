...
i = strmatch('OBJ_DIST',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 4
...
%%%% Default Object Elevation.
i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 4
    def_obj_el = within.parameters(cntr);
else
...
% % % %%%% Setting the new object trajectory direction.
% % % new_obj_ele = def_obj_el + obj_dev;
% % % %%%%new_obj_ele = def_obj_el + obj_dev + atand(9.19/5.81);%%% atand(dist(2,1)*cos(extrael(1)*3.14/180)/obj_dist); %%%%
...
%%% Calculating the object trajectory vector.
...
if (activeStair == 4) %%% Vest + UP-MOVING-Object
    ...
    i = strmatch('OBJECT_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = def_obj_el + obj_dev;
    setappdata(appHandle,'protinfo',data);
...
if (activeStair == 6)  %%%% Vest + Visual + UP-MOVING-Object
    ...
    setappdata(appHandle,'protinfo',data);   i = strmatch('OBJECT_POS', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = [-5 10 0];
    setappdata(appHandle,'protinfo',data);
end
...
if (activeStair == 7) %%% Vest + DOWN-MOVING-Object
    ...
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 270; %%%% OBJECT MOVES UP = 270.
    setappdata(appHandle,'protinfo',data);
