...
% % lateralM(length(dM1),1) = 0;
% % surgeM(length(dM1),1) = 0;
% % heaveM(length(dM1),1) = 0;
% % lateralGL(length(dM1),1) = 0;
% % surgeGL(length(dM1),1) = 0;
% % heaveGL(length(dM1),1) = 0;
...
if motiontype == 1
    ...
    M(13).data = 5.*dM1(1:end-1)/max(abs(dM1));
