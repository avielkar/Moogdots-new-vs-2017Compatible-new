...
if data.configinfo(i).status == 4
    extrael(1,1) = within.parameters.moog(cntr);
    extrael(2,1) = within.parameters.openGL(cntr);
end
extrael
self_dist = dist(2,1);
...
% %     %%%% The desired  motion achieved by changing the ref elevation plane.
% %     %%%% 2nd heading direction is not really used. As the ref elevation
% %     %%%% plane is being manipulated, the 2nd heading is made same as the
% %     %%%% 1st heading. 
% %         amps(1,2) = amps(1,1);
% %         amps(2,2) = amps(2,1); %%%% Making heading direction in both intervals as zero.
...
%%%%% Reading in TO-BE-PERCEIVED Object distance.
i = strmatch('PERC_OBJ_DIST',{char(data.configinfo.name)},'exact');
if data.configinfo(i).status == 4
    perc_obj_dist = within.parameters(cntr)
else
    perc_obj_dist = data.configinfo(i).parameters;
end

%%%% Object deviation with respect to the above-specified reference.
...
if (activeStair == 1) %%%% Vest alone.
    ...
    setappdata(appHandle,'protinfo',data);
    obj_dist = perc_obj_dist;
    i = strmatch('SET_OBJ_DIST',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = obj_dist;
    setappdata(appHandle,'protinfo',data);
end
...
if (activeStair == 4) %%% Vest + UP-MOVING-Object
    i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 0;   %%%% BACKGROUND OFF.
    setappdata(appHandle,'protinfo',data);
    ...
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 90; %%%%%% OBJECT MOVES UP = 90.
    setappdata(appHandle,'protinfo',data);
    
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    def_obj_el = data.configinfo(i).parameters;
    
    x_objdist = perc_obj_dist + self_dist*sin(extrael(1)*3.14/180);
    reqd_obj_dist = sqrt(x_objdist^2 + self_dist*self_dist*cos(extrael(1)*3.14/180)*cos(extrael(1)*3.14/180));
    self_objcomp = -atand(self_dist * cos(extrael(1)*3.14/180)/x_objdist);
    obj_dist = reqd_obj_dist;
    
    i = strmatch('OBJECT_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = def_obj_el + obj_dev + self_objcomp;
    setappdata(appHandle,'protinfo',data);
    i = strmatch('OBJECT_POS', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = [-5 10 0];
    setappdata(appHandle,'protinfo',data);
    
    i = strmatch('SET_OBJ_DIST',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = reqd_obj_dist;
    setappdata(appHandle,'protinfo',data);
end

if (activeStair == 5) %%%% Visual + UP-MOVING-Object
    dist(1,1) = 0.000001;
    dist(1,2) = 0.000001;
    i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 1;
    ...
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 90;
    setappdata(appHandle,'protinfo',data);
...
if (activeStair == 6)  %%%% Vest + Visual + UP-MOVING-Object
    i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 1;
    setappdata(appHandle,'protinfo',data);
...
if (activeStair == 7) %%% Vest + DOWN-MOVING-Object
    i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 0;  %%%%% BACKGROUND IS OFF.
    setappdata(appHandle,'protinfo',data);
    
    i = strmatch('SPHERE_FIELD_PARAMS',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters(1) = 2; %%%%% OBJECT IS ON.
    setappdata(appHandle,'protinfo',data);
    
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 270;  %%%% OBJECT MOVES DOWN = 270.
    setappdata(appHandle,'protinfo',data);
    
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    def_obj_el = data.configinfo(i).parameters; %%% Reading in the above-set value.
    
    x_objdist = perc_obj_dist - dist(2,1)*sin(extrael(1)*3.14/180)
    reqd_obj_dist = sqrt(x_objdist^2 + self_dist*self_dist*cos(extrael(1)*3.14/180)*cos(extrael(1)*3.14/180))
    self_objcomp = atand(self_dist*cos(extrael(1)*3.14/180)/x_objdist);
    obj_dist = reqd_obj_dist;
    
    i = strmatch('OBJECT_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = def_obj_el + obj_dev + self_objcomp; %%%% COMPENSATING FOR SELF-MOTION.
    setappdata(appHandle,'protinfo',data); 
    i = strmatch('OBJECT_POS', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = [-5 -10 0]; %%%% SETTING LOCATION OF OBJECT.
    setappdata(appHandle,'protinfo',data);
    i = strmatch('SET_OBJ_DIST',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = reqd_obj_dist;
    setappdata(appHandle,'protinfo',data);
...
if (activeStair == 8) %%%% Visual + DOWN-MOVING-Object
    dist(1,1) = 0.000001;
    dist(1,2) = 0.000001;
    ...
    i = strmatch('DEF_OBJ_ELE',{char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 270;
    setappdata(appHandle,'protinfo',data);
...
if (activeStair == 9)  %%%% Vest + Visual + DOWN-MOVING-Object
    i = strmatch('BACKGROUND_ON', {char(data.configinfo.name)},'exact');
    data.configinfo(i).parameters = 1;
    setappdata(appHandle,'protinfo',data);
...
%%% Constructing the object trajectory vector.
...
if motiontype == 1    
    ...
    %%%% Desired motion acheived by changing the ref ele plane. This will
    %%%% be different from the el plane used in the first interval. So the
    %%%% value obtained from the first interval is cleared and new value
    %%%% created  by adding the deviation. 
% %     clear el  
% %     el = (elP + dev)*pi/180;
