...
obj_az = 0;
def_obj_el = 270;
obj_dist = 1.0*dist(2,1); %%% Object distance same as movement distance.
...
% % %%% orig_xpos = data.configinfo(i).parameters(1)
% % clear temp
% % temp = rand(1,1);
% % new_xpos = -30 + 10*temp
% % data.configinfo(i).parameters(1) = new_xpos;
% % setappdata(appHandle,'protinfo',data);
...
if (activeStair == 1) %%% Pure object motion.
    %%% No self-motion and no optic flow. Obj motion in world coordinates.
    dist(1,1) = 0.00001;
    dist(2,1) = 0.00001;
...
if (activeStair == 2)
    %%%% Object motion + vestibular stim (no optic flow)
    dist(1,1) = 13;
    dist(2,1) = 13;  %%% this cannot be zero.
...
if (activeStair == 3) 
    %%% Object motion + optic flow only (no vest stimulus)
    dist(1,1) = 0.00001;
    dist(2,1) = 13;
...
if (activeStair == 4)
    %%%%%% Object motion + optic flow + vest stimulus
    dist(1,1) = 13;
    dist(2,1) = 13;
