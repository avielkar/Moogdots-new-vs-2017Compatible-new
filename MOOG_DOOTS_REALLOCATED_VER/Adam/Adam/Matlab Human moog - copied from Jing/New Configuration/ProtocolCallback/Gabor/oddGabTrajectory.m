...
% odd trajectory
if motiontype == 3    
    vec_sum_dist=sqrt(dist(1,1)^2 + dist(1,2)^2); % modify distance for vector sum stimulus: sqrt(pedestal_mag^2 + increment_mag^2)
    vMod = GenGabor(dur(1,2), sig(1,2), vec_sum_dist, f);
    dMod = cumtrapz(vMod);
...
if motiontype == 1 % We should never enter this one
    ...
    % moify elevation for vector sum, "odd" stimulus: atan(pedestal_mag/increment_mag)
    ang_inc=atan2(dist(1,1),dist(1,2))/pi*180;
    el=(elP+ang_inc)*pi/180; 
