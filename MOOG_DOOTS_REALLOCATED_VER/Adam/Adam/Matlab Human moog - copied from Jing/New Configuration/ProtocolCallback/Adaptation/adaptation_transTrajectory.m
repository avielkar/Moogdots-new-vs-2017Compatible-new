function [M] = adaptation_transTrajectory(appHandle)

global debug

if debug
    disp('Entering Adaptation transTrajectory');
end
...
if motiontype == 3 % 2I vars required as well
    ...
    %============================================================
    %Something wrong here, but we don't care the 2nd Interval
    i = strmatch('ADAPTATION_ANGLE',{char(data.configinfo.name)},'exact');
    if data.configinfo(i).status == 2
        i1 = strmatch('Adaptation Angle',{char(data.condvect.name)},'exact');
        adaptation_amp(1,1) = crossvals(cntr,i1);
    else
...
el=elP*pi/180;
tilt=tiltP*pi/180;
adaptation_amp = adaptation_amp*pi/180; %Adaptation Angle offset for visual stimulus
...
yGL = sin(adaptation_amp + amp(2,ord(1)))*cos(az(2))*cos(tilt(2)) + cos(adaptation_amp + amp(2,ord(1)))*...
...
if debug
    ...
    disp('Exiting Adaptation transTrajectory');
end
