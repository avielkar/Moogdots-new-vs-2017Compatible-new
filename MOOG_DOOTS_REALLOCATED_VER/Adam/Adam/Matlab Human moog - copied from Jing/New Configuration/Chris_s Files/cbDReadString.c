...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 3) {
        mexErrMsgTxt("*** Usage:  [stringVal] = cbDReadString(boardNum, timeOut, buffSize)");
    }
    ...
    errorCode = cbDIn(boardNum, FIRSTPORTB, &recVal);
    if (recVal) {
        #if VERBOSE_MODE
        ...
        do {
            ...
            do {
                if (cbDIn(boardNum, FIRSTPORTB, &recVal) != 0) {
            ...
            if (cbDOut(boardNum, SECONDPORTA, (unsigned short)0) != 0) {
